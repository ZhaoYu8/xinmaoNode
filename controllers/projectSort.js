...
let obj = {
  ...
  'POST /delSort': async (ctx, next) => {
    let param = ctx.request.body
    let data = await connection.query(`delete from projectSort ta where ta.id = '${param.id}'`)
    ctx.body = (Object.assign(global.createObj(), { item: data }))
  },
  'POST /querySort': async (ctx, next) => {
    let param = ctx.request.body
    let data = await connection.query(`select ta.* from projectSort ta where ta.company = '${param.company}' ${param.name ? `and ta.name = ${param.name}` : ''}`)
    ctx.body = (Object.assign(global.createObj(), { item: data }))
  },
  'POST /editSort': async (ctx, next) => {
    let param = ctx.request.body
    let data = await connection.query(`update projectSort set name = "${param.name}", parent = "${param.parent}" where id = "${param.id}"`)
    ctx.body = (Object.assign(global.createObj(), { item: data }))
  }
}
module.exports = obj;